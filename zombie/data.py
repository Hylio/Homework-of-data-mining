...
base_train = pd.read_csv("classify/base-train.csv", encoding='ANSI')
knowledge_train = pd.read_csv("classify/knowledge-train.csv", encoding='ANSI')
money_train = pd.read_csv("classify/money-train.csv", encoding='ANSI')
year_train = pd.read_csv("classify/year-train.csv", encoding='ANSI')
...
money_year_train_15_16_17 = pd.merge(money_year_train_15_16, money_year_train_17, on='ID')
train_total_data = pd.merge(money_year_train_15_16_17, base_knowledge_train, on='ID')

# 因为具体年份已经可以通过加后缀得以区分了
train_total_data = train_total_data.drop(columns=['year_x', 'year_y', 'year'])
...
# 增加一些特征以便更好分类
train_df = pd.DataFrame(train_total_data)
train_df['利润营收比_2015'] = train_df['利润总额_2015']/(train_df['营业总收入_2015']+1)
train_df['利润营收比_2016'] = train_df['利润总额_2016']/(train_df['营业总收入_2016']+1)
train_df['利润营收比_2017'] = train_df['利润总额_2017']/(train_df['营业总收入_2017']+1)
train_df['政府依赖度_2015'] = train_df['项目融资和政策融资额度_2015']/(train_df['营业总收入_2015']+1)
train_df['政府依赖度_2016'] = train_df['项目融资和政策融资额度_2016']/(train_df['营业总收入_2016']+1)
train_df['政府依赖度_2017'] = train_df['项目融资和政策融资额度_2017']/(train_df['营业总收入_2017']+1)

train_df.to_csv("classify/train.csv", encoding='utf8')


# train_total_data.to_csv("classify/train.csv", encoding='utf8')
